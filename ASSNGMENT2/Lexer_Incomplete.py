...
class Lexer():
    ...
    def _possible_tokens(self, char_stream:List[str]) -> List[Token]:
        ...
        character_Check = Lexeme_Multi_Char(self._lexemes, char_stream)
        for x in char_stream:
            character_Check.feed(x)

            print(character_Check.get_state())
            print(character_Check.get_captured())
            print(character_Check.get_token())
